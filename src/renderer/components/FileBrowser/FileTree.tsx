...
export function FileTree() {
  ...
  const loadFileTree = useVaultStore((state) => state.loadFileTree);
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header with vault info and refresh button */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-gray-700 bg-gray-800">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <span className="text-xs text-gray-300 truncate" title={vaultPath || undefined}>
            {vaultName}
          </span>
        </div>
        <button
          onClick={loadFileTree}
          className="flex items-center gap-1 px-2 py-1 text-xs text-gray-400 hover:text-white hover:bg-gray-700 rounded transition-colors flex-shrink-0"
      ...
      <div className="flex-1">
        <Tree
          ...
          openByDefault={false}
          width="100%"
          height={window.innerHeight - HEADER_HEIGHT - 36}
          indent={24}
          rowHeight={32}
        >
          {Node}
